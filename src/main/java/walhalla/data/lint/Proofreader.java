...
public class Proofreader {
    ...
    public static final Linter LINTER = new Linter()
            ...
            .normalize("(常時)", "")
            .normalize("タイプ", "属性")
            .normalize("優先攻撃", "優先的に攻撃")
            .normalize("優先して", "優先的に")
            .normalize("自身のトークン", "自トークン")
            .normalize("ブロック中の", "ブロックした")
            .normalize("ブロックしている", "ブロックした")
            .normalize("味方がブロックした敵を優先的に攻撃と攻撃と", "味方がブロックした敵を優先的に攻撃、攻撃力と") // ダルティエのtypo修正

            // Token rules
            .addRule("敵を倒すとまれにゴールドを入手するトークンを使役", "\nトークンを使役する[@ブロック数：1@敵を倒すとまれにゴールドを入手]\n") // マーチャント系
            .addRule("敵を倒すとまれにゴールドを入手できるトークンを使役", "\nトークンを使役する[@ブロック数：1@敵を倒すとまれにゴールドを入手]\n") // マーチャント系
            .addRule("ブロック数2で敵を倒すとまれにゴールドを入手するトークンを使役", "\nトークンを使役する[@ブロック数：2@敵を倒すとまれにゴールドを入手@毒・状態異常を無効化]\n") // マーチャント系
            .addRule("遠距離攻撃を行い敵を倒すとまれにゴールドを入手するトークンを使役", "\nトークンを使役する[@ブロック数：1@敵を倒すとまれにゴールドを入手@遠距離攻撃を行う]\n") // マーチャント系
            .addRule("出撃数に含まれず、敵の遠距離攻撃を引き付けるスキルを持つトークンを使役", "\nトークンを使役する[@出撃人数に含まれない@敵の遠距離攻撃を引き付ける]\n") // メシナ
            .addRule("出撃数に含まれず、毒・状態異常を無効化しスキルを持つトークンを使役", "\nトークンを使役する[@出撃人数に含まれない@毒・状態異常を無効化@スキルを使用する]\n") // 正月メシナ
            .addRulePattern("遠距離攻撃の対象にならず出撃数に含まれない[特殊]トークンを使役", "\nトークンを使役[@遠距離攻撃の対象にならない@出撃人数に含まれない]\n") // 正月メシナ
            .addRule("遠距離攻撃の対象にならず、範囲内の敵に攻撃力に比例した貫通属性の小継続ダメージを与え、2体まで配置可能で1体まで出撃人数に含まれないトークンを使役", "トークンを使役[@敵の遠距離攻撃の対象にならない@範囲内の敵に攻撃力に比例した貫通属性の小継続ダメージを与える@2体まで配置可能@1体まで出撃人数に含まれない]\n") // コハル
            .addRule("遠距離攻撃の対象にならず、範囲内の敵に攻撃力に比例した貫通属性の小継続ダメージを与え、1体まで配置可能で1体まで出撃人数に含まれないトークンを使役", "トークンを使役[@敵の遠距離攻撃の対象にならない@範囲内の敵に攻撃力に比例した貫通属性の小継続ダメージを与える@1体まで配置可能@1体まで出撃人数に含まれない]\n") // コハル
            .addRule("HPが徐々に減少するが遠距離攻撃の対象にならず範囲内の敵の防御と魔法耐性を40%減少し、出撃数に含まれない特殊トークンを使役", "\nトークンを使役する[@HPが徐々に減少する@範囲内の敵の防御と魔法耐性を40%減少@遠距離攻撃の対象にならない@出撃人数に含まれない]\n") // ヘルメルク
            .addRule("HPが徐々に減少するが遠距離攻撃の対象にならず範囲内の敵の防御と魔法耐性を50%減少し、出撃数に含まれない特殊トークンを使役", "\nトークンを使役する[@HPが徐々に減少する@範囲内の敵の防御と魔法耐性を50%減少@遠距離攻撃の対象にならない@出撃人数に含まれない]\n") // ヘルメルク
            .addRule("スキルを持ち敵を足止めせず、遠距離攻撃を行うトークンを使役、トークンはHPが高速減少するが、遠距離攻撃の対象にならない", "トークンを使役する[@HPが高速に減少する@遠距離攻撃を行う@敵の遠距離公的の対象にならない@スキルを使用する]") // リタ
            ...
            .addRule("上記ユニット死亡時、撤退扱い", "\n対象ユニットの死亡時は撤退として扱う\n")
            .addRule("上記ユニット死亡時は撤退扱い", "\n対象ユニットの死亡時は撤退として扱う\n")
            .addRule("上記ユニット死亡時、撤退として扱う", "\n対象ユニットの死亡時は撤退として扱う\n")
            .addRule("上記ユニット死亡時は撤退として扱う", "\n対象ユニットの死亡時は撤退として扱う\n")
            .addRule("上記ユニットが死亡した場合、撤退として扱う", "\n対象ユニットの死亡時は撤退として扱う\n")
            .addRule("上記ユニットが死亡した場合は撤退として扱う", "\n対象ユニットの死亡時は撤退として扱う\n")
            .addRule("主人が死亡した場合撤退扱い", "\n対象ユニットの死亡時は撤退として扱う\n")
            .addRule("自身が死亡した時撤退として扱う", "\n死亡時は撤退として扱う\n")
            .addRule("死亡時撤退扱い", "\n死亡時は撤退として扱う\n")
            .addRule("死亡時、撤退扱い", "\n死亡時は撤退として扱う\n")
            .addRule("死亡時、撤退扱い。", "\n死亡時は撤退として扱う\n")
            .addRule("自身が死亡した場合、撤退として扱う", "\n死亡時は撤退として扱う\n")
            ...
            .addRulePattern("[地上の|味方がブロックした|自トークンがブロックした|トークンがブロックした|バリケードがブロックした|アンデッド属性の|アーマー属性の]敵を優先的に攻撃[し]", "\n$0敵を優先的に攻撃する\n")
            .addRule("スキルを自動で使用する", "\nスキルを自動で使用する\n")
            .addRulePattern("[攻撃力|防御力|最大HP]の高いユニットを優先的に支援", "\n$0の高いユニットを優先的に支援\n")
            .addRule("自トークンが敵の遠距離攻撃の対象にならない", "\n自トークンは敵の遠距離攻撃の対象にならない\n")
            .addRule("全味方が遠距離攻撃の対象にならない", "\n全ユニットは敵の遠距離攻撃の対象にならない\n")
            .addRule("全味方に敵の遠距離攻撃の対象にならない効果を付与", "\n全ユニットは敵の遠距離攻撃の対象にならない\n")
            .addRulePattern("[範囲内]の[味方]ユニット[が][は][、][敵の]遠距離攻撃の対象になら[ない][ず][、]", "\n範囲内のユニットは敵の遠距離攻撃の対象にならない\n")
            .addRulePattern("スキル中[は][の][、][範囲内]の[全][味方]ユニット[が][は][、][敵の]遠距離攻撃の対象になら[ない][ず][、]", "\nスキル中は範囲内のユニットが敵の遠距離攻撃の対象にならない\n")
            .addRulePattern("スキル[使用時][中][のみ][は][、][敵の]遠距離攻撃の対象になら[ない][ず][、]", "\nスキル中は敵の遠距離攻撃の対象にならない\n")
            .addRulePattern("[非]スキル[未使用時][中][は][、][敵の]遠距離攻撃[の対象にならない][対象外]", "\n非スキル中は敵の遠距離攻撃の対象にならない\n")
            .addRulePattern("[敵][の]遠距離攻撃の対象になら[ない][ず]", "\n敵の遠距離攻撃の対象にならない\n")
            .addRule("敵の遠距離攻撃の対象にならず、範囲内の敵の", "\n敵の遠距離攻撃の対象にならず範囲内の敵の") // ニミュエちゃんのクラス用
            .addRule("スキル中、敵の遠距離攻撃の対象にならず、範囲内の敵の", "\nスキル中、敵の遠距離攻撃の対象にならない\nスキル中、範囲内の敵の") // ユージェンのアビリティ
            .addRulePattern("敵[の]遠距離攻撃を[自身に]引き付ける[が][、]", "\n敵の遠距離攻撃を引き付ける\n")
            .addRulePattern("スキル中、敵[の]遠距離攻撃を[自身に]引き付け[る][が][、]", "\nスキル中は敵の遠距離攻撃を引き付ける\n")
            ...
            .addRule("自トークンが敵の遠距離攻撃の対象にならず", "\n自トークンは敵の遠距離攻撃の対象にならず") // 嫁レナータのスキル
            ...
            .addRule("周囲の敵をブロック数0で高速攻撃", "\nブロック数：0\n周囲の敵を高速攻撃\n")
            .addRule("自身はブロック数0で遠距離攻撃", "\nブロック数：0\n遠距離攻撃を行う\n") // アリエル
            .addRule("発射する矢の速度が上昇", "\n発射する矢の速度が上昇\n")
            ...
            .addRulePattern("敵に止めを刺さない[が]", "\n敵に止めを刺さない\n")
            .addRule("戦闘中1回しか使用できない", "\n戦闘中1回しか使用できない\n")
            .addRule("戦闘中1回しか使えない", "\n戦闘中1回しか使用できない\n")
            ...
            .addRule("次回スキル強化", "\n次回スキルが変化\n")
            .addRule("次回スキル変化", "\n次回スキルが変化\n")
            .addRule("王子を配置中、出撃コスト-3", "\n王子を配置している間は自身の出撃コストが減少する(-3)")
            ...
            .addRegex("(範囲内の味方の)?(トークンと自身が)?(自身の)?毒・状態異常を(完全に)?無効化(することができる|し、)?", "\n$1毒・状態異常を無効化\n")
            ...
            .addRegex("ブロック数(\\d)[、で]([^H攻防魔])", "\nブロック数：$1\n$2")
            .addRegex("(敵)?(\\d)体まで(の敵)?を足止め(できる|でき|し)", "\nブロック数：$2体\n")
            .addRegex("編成([中|時])", "\n編成$1")
            .addRegex("出撃([中|時])", "\n出撃$1")
            .addRegex("([^くンを])配置([中|時])(、)?\\n?", "$1\n配置$2$3")
            .addRegex("([^+])効果時間無限", "$1\n効果時間無限")
            .addRegex("^スキル発動時、出撃コスト\\+(\\d+)", "スキル発動時に出撃コストが回復($1)\n")
    ...
    public static String fix(String text, String desc) {
        return LINTER.fix(text, desc).text.strip();
    }
